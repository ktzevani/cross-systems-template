...
$Profile = Join-Path $ProjectRoot "profiles/windows-msvc-msbuild"
$OutputFolder = Join-Path $ProjectRoot "out/conan/windows-msvc-msbuild"

foreach ($Config in $Configurations) {
    $RuntimeType = if ($Config -eq "Debug") { "Debug" } else { "Release" }

    Write-Host "[conan] Installing Windows MSVC/MSBuild dependencies for $Config"
...
Write-Host "[conan] Windows MSVC/MSBuild dependency metadata is ready: $OutputFolder"
